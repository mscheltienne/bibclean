...
def _check_config(required_fields, keep_fields) -> None:
...
def test_load_config(assets: Path) -> None:
    """Test loading of a TOML configuration."""
    req_def, keep_def = _load_default_config()

    # valid
    req, keep, exclude = load_config(assets / "config" / "valid.toml")
    ...
    req, keep, exclude = load_config(assets / "config" / "valid-2.toml")
    assert len(req) == len(keep) == len(req_def) == len(keep_def)
    ...
    # valid-no-book
    req, keep, exclude = load_config(assets / "config" / "valid-no-book.toml")
    ...
    # valid-exclude-type
    req, keep, exclude = load_config(assets / "config" / "valid-exclude-type.toml")
    assert len(req) == len(keep) == len(req_def) == len(keep_def)
    assert req["article"] == {"author", "journal", "title", "year"}
    assert req["book"] == {"author", "publisher", "title", "year"}
    assert keep["article"] == {"author", "journal", "title", "issn", "year"}
...
def test_load_invalid_config(assets: Path) -> None:
    ...
    with pytest.raises(
        ...
        load_config(assets / "config" / "invalid-missing-tool.toml")
    with pytest.raises(
        RuntimeError,
        match=re.escape("invalid. The section(s) 'tool.bibclean' are missing."),
    ):
        load_config(assets / "config" / "invalid-missing-section.toml")

    # invalid keys
    with pytest.raises(KeyError, match=re.escape("invalid. Unexpected ['author'] key")):
        load_config(assets / "config" / "invalid-keys-singular.toml")
    with pytest.raises(
        KeyError,
        match=re.escape("invalid. Unexpected ['author', 'year'] keys"),
    ):
        load_config(assets / "config" / "invalid-keys-plural.toml")
    ...
    with pytest.raises(
        ...
        load_config(assets / "config" / "invalid-exclude.toml")
    with pytest.raises(
        TypeError, match="The excluded types must be provided in str format"
    ):
        load_config(assets / "config" / "invalid-exclude-type.toml")

    # duplicated fields
    with pytest.raises(
        ValueError, match="invalid. Some fields are present multiple times"
    ):
        load_config(assets / "config" / "invalid-duplicate-fields.toml")


def test_invalid_merge(assets: Path) -> None:
    """Test invalid merge when default configuration is absent."""
    with pytest.raises(
        RuntimeError,
        match="'tool.bibclean.test' does not have a 'required' default.",
    ):
        load_config(assets / "config" / "invalid-missing-req-def.toml")

    with pytest.raises(
        RuntimeError,
        match="'tool.bibclean.test' does not have a 'keep' default.",
    ):
        load_config(assets / "config" / "invalid-missing-keep-def.toml")
